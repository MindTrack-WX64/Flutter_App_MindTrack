...
class SessionService extends BaseService<SessionEntity> {
  ...
  Future<http.Response> createSessionOfTreatmentPlan(int treatmentPlanId, Map<String, dynamic> sessionData, String token) async {
    // Convert the Map to JSON
    final sessionJson = json.encode(sessionData);

    final response = await http.post(
      Uri.parse('$apiUrl$resourceEndpoint/$treatmentPlanId'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  Future<List<SessionEntity>> findByTreatmentPlanId(int treatmentPlanId, String token) async {
    final response = await http.get(
      Uri.parse('$apiUrl$resourceEndpoint/treatment-plan/$treatmentPlanId'),
      headers: {
        'Authorization': 'Bearer $token',
      },
    );
